...
class Character:
    ...
    def get_highest_aggro(self):
        ...
        if not self.aggro or highest_aggro <= 0:
            return "Aggro: none"
        highest_players = [
            player for player, aggro in self.aggro.items() if aggro == highest_aggro
        ]
        ...
        for damage_type, damage in damage_dict.items():
            if damage_type == "p":
                ...
                if attacker_name and self.tag == "enemy":
                    attacker = next(
                        (c for c in characters if c.name == attacker_name), None
                    )
        ...
        for healing_type, amount in healing_dict.items():
            if healing_type == "h":
                ...
                if self.health + effective_healing > self.max_health:
                    effective_healing = (
                        effective_healing
                        - (self.health + effective_healing) % self.max_health
                    )
...
def show_stats(characters, current_index):
    ...
    for char in characters:
        print(
            f"{char.name} - HP: {char.health}/{char.max_health}, Temp HP: {char.temp_hp}, Ini: {char.initiative}, Armor: {char.armor}, Arcane Resist: {char.arcane_resist}, Light Resist: {char.light_resist}, Nature Resist: {char.nature_resist}, Tag: {char.tag}"
        )
        if char.tag == "enemy":
...
def change_stat(characters):
    ...
    stat = input(
        "Zu ändernder Wert (health/temp_hp/initiative/armor/arcane_resist/light_resist/nature_resist/aggro): "
    )
    if stat == "aggro":
...
def confirm_action(message):
    confirmation = input_with_retry(
        f"{message} Bestätigen? (j/n): ", str, ["j", "n"]
    ).lower()
    ...
    while True:
        try:
            ...
            if allowed_values and user_input not in allowed_values:
                raise ValueError(
                    f"Eingabe muss eine der folgenden sein: {allowed_values}"
                )
            return user_input
...
def battle_round(characters, round_counter):
    ...
    while current_index < len(characters):
        ...
        while True:
            action_type = input_with_retry(
                "Aktion (a/h/e/adm): ", str, ["a", "h", "e", "adm"]
            ).lower()
            ...
            if action_type == "a":  # Attack
                ...
                damage_input = input(
                    "Schaden eingeben (Zahl + p:physical/a:arcane/l:light/n:nature/g:Aggro): "
                )
                ...
                if target_name == "all":
                    ...
                    target = next(
                        (c for c in characters if c.name == target_name), None
                    )
                ...
                for target in targets:
                    if target and char.tag == target.tag:
                        if not confirm_action(
                            f"{char.name} fügt {target.name} Schaden zu"
                        ):
                            continue

                    if target:
                        actual_damage = target.calculate_damage(
                            damage_dict, char.name, characters
                        )
                        print(
                            f"{char.name} verursacht {actual_damage} Schaden an {target.name}."
                        )
                        print(
                            f"{target.name} hat jetzt noch {target.health} Gesundheit und {target.temp_hp} temporäre HP."
                        )
                    else:
                ...
                target_name = input("Ziel (Name oder 'all'): ")
                healing_input = input(
                    "Heilungsmenge eingeben (Zahl + h:healing/t:temp_hp): "
                )
                ...
                for target in targets:
                    ...
                    if target:
                        actual_healing, actual_temp_hp = target.heal(
                            healing_dict, char.name, characters
                        )
                        print(
                            f"{char.name} heilt {target.name} um {actual_healing} HP und gibt {actual_temp_hp} temporäre HP."
                        )
                        print(
                            f"{target.name} hat jetzt {target.health} Gesundheit und {target.temp_hp} temporäre HP."
                        )
                    else:
                        print("Ziel nicht gefunden. Aktion übersprungen.")
                ...
                admin_command = input_with_retry(
                    "Admin-Befehl (show/change_stat/change_active_char/save/load/create/delete): ",
                    str,
                    [
                        "show",
                        "change_stat",
